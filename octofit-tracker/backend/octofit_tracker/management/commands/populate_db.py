...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        Activity.objects.create(user='spiderman', activity_type='run', duration=25)

        # Crear leaderboard
        Leaderboard.objects.create(user='ironman', points=100)
        Leaderboard.objects.create(user='batman', points=90)
        Leaderboard.objects.create(user='superman', points=110)
        Leaderboard.objects.create(user='spiderman', points=95)

        # Crear workouts
        Workout.objects.create(name='Cardio Blast', description='Intenso cardio de 30 minutos')
        Workout.objects.create(name='Strength Circuit', description='Rutina de fuerza de 45 minutos')
        Workout.objects.create(name='Swim Power', description='Entrenamiento de natación de 60 minutos')
